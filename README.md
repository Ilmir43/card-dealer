...
На странице «Распознавание» есть флажок «Распознавать с камеры», который
позволяет обрабатывать поток с подключённой камерой в реальном времени. Здесь
же расположена кнопка «Следующая карта», запускающая сервопривод для выдачи
очередной карты.
