...
## Пошаговая раздача по фотографиям

Скрипт `streamlit_deal_photos.py` позволяет загружать готовые снимки карт и
распознавать их по одной, немедленно раздавая игрокам. После загрузки набора
изображений нажимайте кнопку «Распознать и раздать», чтобы текущая карта была
определена и передана очередному игроку.

```bash
streamlit run streamlit_deal_photos.py
```

Количество игроков задаётся в интерфейсе. Карты распределяются последовательно
