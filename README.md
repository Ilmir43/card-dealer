...
Классификатор (`classifier.py`) использует веса, сохранённые в отдельном файле
формата `.pt`. Этот файл содержит параметры нейронной сети, определяющей ранг и
масть карты. Вы можете обучить классификатор скриптом `train_model.py` и
указать путь к полученному файлу в поле *Файл классификатора* на странице
Streamlit.
