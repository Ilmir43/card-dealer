...
Сначала сформируйте базу эмбеддингов для всех изображений в `datasets/cards`:

```bash
python generate_embeddings.py --data datasets/cards --output embeddings.pkl
```
...
изображения в папки `train` или `val` и заново запуская скрипт.
...
Укажите путь к файлу `embeddings.pkl` и загрузите изображение или видео.
Приложение покажет предполагаемые карты в порядке появления.
